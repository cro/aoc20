defmodule Aoc04Test do
  ...
  test "Passport Parse, just keys" do
    parsed_passports = Aoc04.read_file("day04-test.txt")
    |> Aoc04.split_file
    |> Aoc04.join_file("",[])
    |> Aoc04.list_of_maps

    assert length(parsed_passports) == 12
    valid = Enum.count(parsed_passports, fn x -> (Map.keys(x) == ["byr","cid","ecl","eyr","hcl","hgt","iyr","pid"] or
        Map.keys(x) == ["byr","ecl","eyr","hcl","hgt","iyr","pid"]) end)
    assert valid == 10
  end
